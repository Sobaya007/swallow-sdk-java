...
public class SwallowTest implements Swallow {
	private int dummyTime = (int)(new Date().getTime() / 1000) - 100000;
	private int getDummyTime() {
		return dummyTime += 123;
	};
	...
	private ArrayList<Swallow.User> dummyUser = new ArrayList<Swallow.User>(Arrays.asList(new Swallow.User[]{
		...
		new Swallow.User(3, getDummyTime(), "あいうえお", "ユーザ名がマルチバイトな人", 2),
		new Swallow.User(4, getDummyTime(), "かきくけこかきくけこかきくけこかきくけこかきくけこかきくけこかきくけこかきくけこかきくけこかきくけこかきくけこかきくけこかきくけこ", "名前がムダに長い人", 3),
		new Swallow.User(5, getDummyTime(), "じぶん", "テストユーザ", null),
	}));
	...
	@Override
	public User modifyUser(String password, String userName,
			String profile, Integer imageFileID, String web,
			String twitter, String gcm, String apns,
			Integer[] observeTagIDs, Boolean observeMention) {
		...
		for(Swallow.User entry : dummyUser){
			if(entry.getUserID() == 5){
				list.add(new Swallow.User(entry.getUserID(), (int) (System.currentTimeMillis() / 1000), userName != null ? userName : entry.getUserName(), profile != null ? profile : entry.getProfile(), imageFileID != null ? imageFileID : entry.getImage()));
				det = new Swallow.UserDetail(entry.getUserID(), (int) (System.currentTimeMillis() / 1000), userName != null ? userName : entry.getUserName(), profile != null ? profile : entry.getProfile(), imageFileID != null ? imageFileID : entry.getImage(), web, twitter, gcm, apns, observeTagIDs, observeMention);
			}else{
				list.add(entry);
			}
		}
	...
	public Message createMessage(String message, String[] attributes,
		...
		Swallow.Message newm = new Swallow.Message(1 + dummyMessage.size(), (int) (System.currentTimeMillis() / 1000), 5, message, 0, attributes, fileIDs, tagIDs, replyPostIDs, destUserIDs, enquetes);
	...
	public File createFile(String fileName, String fileType,
		...
		Swallow.File newf = new Swallow.File(1 + dummyFile.size(), (int) (System.currentTimeMillis() / 1000), fileName, fileType, tagIDs, folderContent);
	...
	public Tag[] findTag(Integer startIndex, Integer endIndex,
			Date startDate, Date endDate, Integer minPostNum,
			Integer maxPostNum, String tagNamePattern) {
		return dummyTag.toArray(new Swallow.Tag[0]);
	}
	@Override
	public Tag createTag(String tagName) {
		Swallow.Tag newt = new Swallow.Tag(1 + dummyTag.size(), (int) (System.currentTimeMillis() / 1000), tagName, 0);
		dummyTag.add(newt);
		return newt;
	}
	@Override
	public Favorite[] findFavorite(Integer startIndex, Integer endIndex,
			Date startDate, Date endDate, Integer minFavNum,
	...
	public Favorite createFavorite(Integer postID, Integer favNum) {
		Swallow.Favorite fav = new Swallow.Favorite(5, postID, (int) (System.currentTimeMillis() / 1000), favNum);
		dummyFavorite.add(fav);
		return fav;
	}
	@Override
	public Answer[] findAnswer(Integer startIndex, Integer endIndex,
			Date startDate, Date endDate, Integer[] userIDs,
			Integer[] postIDs) {
	...
	public Answer createAnswer(Integer postID, String answer) {
		...
		for(Swallow.Answer entry : dummyAnswer){
			if(entry.getUserID() == 5){
				newa = new Swallow.Answer(5, postID, (int) (System.currentTimeMillis() / 1000), answer);
				list.add(newa);
			}else{
				list.add(entry);
			}
	...
	public Received createReceived(Integer postID) {
		...
		for(Swallow.Received entry : dummyReceived){
			if(entry.getUserID() == 5){
				newr = new Swallow.Received(5, postID, (int) (System.currentTimeMillis() / 1000));
				list.add(newr);
			}else{
				list.add(entry);
			}
